...
db_exists() {
  ...
  $EXEC_COMMAND $DB_CONTAINER psql -h $DB_CONTAINER -U $DB_USER -lqt | cut -d \| -f 1 | grep -qw $DB_NAME
}

# Function to create the database
create_db() {
  echo_yellow "Creating database: $DB_NAME"
  $EXEC_COMMAND $DB_CONTAINER psql -h $DB_CONTAINER -U $DB_USER -c "CREATE DATABASE $DB_NAME"
}

# Function that verifies required input was passed in
verify_input() {
  # Verify required inputs are not empty
  [ ! -z "$DB_CONTAINER" ] && [ ! -z "$DB_NAME" ] && [ ! -z "$DB_USER" ]
}
...
# Main function
main() {
  echo_yellow "Creating postgres database..."
...
# Verify input
! verify_input && echo_red "Missing script options" && usage && exit 1
